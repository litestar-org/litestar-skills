---
name: litestar-vite
description: "Auto-activate for litestar_vite imports, VitePlugin, ViteConfig, vite.config.ts, vite.config.js, litestar-vite-plugin (npm), `litestar assets` CLI commands. The first-party plugin coordinating a Vite frontend with a Litestar backend across SPA, template, HTMX, Inertia, and SSR-framework modes. Produces ViteConfig (Python side) + vite.config.ts (JS side) wiring, hot-file management, manifest resolution, type generation (routes.ts / schemas.ts / openapi.json / inertia-pages.json), template helpers (vite_asset, vite_hmr_client, vite_react_refresh), HMR proxy setup, and production build/deploy patterns. Use when: integrating Vite with Litestar, building a frontend (React / Vue / Svelte / HTMX / Inertia), setting up HMR, generating types from the backend, serving static assets in dev or prod, or choosing a frontend mode. Not for Webpack, Rollup, esbuild, or Parcel — litestar-vite requires Vite specifically. Not for plain Vite outside Litestar — use the standalone Vite docs."
---
